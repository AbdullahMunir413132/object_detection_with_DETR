...
## 2. Problem Statement
Developing real-time computer vision applications in purely synchronous frameworks often presents significant architectural bottlenecks. Primarily, running heavy ML inference alongside UI updates caused the Streamlit websocket to freeze, resulting in a blank "white screen of death" or severe lag during video streaming. Furthermore, managing dynamic model loading, environment path handling on Windows, and hardware camera capture natively within centralized Python scripts regularly led to fragile, environment-dependent crashes unsuited for production deployment.

## 3. Our Solution
To overcome these limitations, we devised a fully decoupled, multi-threaded architecture. By shifting the heavy rendering, video encoding, and frame delivery to a standalone, asynchronous FastAPI MJPEG server, the Streamlit process was strictly utilized for its strengths: dashboard parameter control and analytics display. In parallel, we hardened environment dependencies to ensure absolute path resolutions for model weights (`.pt` files) and implemented robust fallback chains for OpenCV's Windows backends, guaranteeing stable hardware stream acquisition.

## 4. Technical Details & Stack
...
## 7. Our Contributions & Improvements
